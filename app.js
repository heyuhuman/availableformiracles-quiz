/* =========================================================
   ...
   - URL param mapping: ?1=utm_source
     e.g. ?1=instagram  ?1=facebook  ?1=tiktok
...
/* ------- URL param mapping ------- */
/* Maps short param keys to their semantic names.
   Add more entries here if you add more tracking params.
   e.g. "2": "utm_medium", "3": "utm_campaign"           */
function getUtmParams() {
  const params = new URLSearchParams(window.location.search);
  return {
    utm_source: params.get("1") || ""
  };
}
...
function buildFormSparkPayloadFlat() {
  ...
  /* ----- UTM / tracking params (mapped from short URL keys) ----- */
  payload.utm_source  = utmParams.utm_source;
  /* To add more params, extend getUtmParams() at the top and add lines here:
...
function renderInterstitialCalc(screen) {
  ...
  finishMsg.textContent = screen.finishMessage || "Your cosmic details have been collected. Let's personalize your report.";
...
function renderInterstitialBubble(screen) {
  ...
  speechBubble.textContent = screen.speech || "Almost done! Share a bit more to reveal what's driving you to get a more accurate reading!";
...
function renderContact(screen) {
  ...
  function validateNow(showErrors = false) {
    ...
    if (!showErrors) { error.textContent = ""; return; }
